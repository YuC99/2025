...
We have got data from Eurostat and opened them using different packages (ggplot2, eurostat,lubridate,dplyr,tidyr).
...
```
Standard deviation combined with mean shows us volatility of the inflation. In PL and CZ difference between mean and sd is high that characterizes 2023 year like a year with periods of extreme prices growth that was replaced with periods of disinflation.
...
```
Median means "typical" level of inflation in countries. In all cases in our examples difference between mean and median is not high that means that there are no outlayers (extremely high or low levels of inf.)
...
```
The difference between max(top level) and min shows range of inflation fluctuations for the year. But in NL inf rate is -1% - there were a month with prices decreases.
...
cor
```
Inflation in Central and Western Europe (Germany, Italy, France, Czech Republic, Poland, Netherlands) moves together, showing strong similarity. Spain is the outlier, with a very different inflation pattern and much lower inflation levels.
...
PL also starts very high, around 17%.
...
ggplot(cor2, aes(x = country1, y = country2, fill = corr)) +
  ...
  theme_minimal()
```
This heatmap shows how similar the inflation movements were between the seven EU countries in 2023. Darker blue means a strong positive correlation (the countries’ inflation moved in the same direction). Lighter areas mean a weaker relationship.
...
In this trend analysis we can see that in all represented countries inflation rate was decreasing - this is the main trend in 2023 year. 

```{r}
stability_analysis <- data2 %>%
  group_by(geo) %>%
  arrange(values) %>%
  summarise(
    mean_inflation = mean(values),
    sd_inflation = sd(values),
    cv_inflation = sd_inflation / mean_inflation * 100
    ) %>%
  arrange(sd_inflation)
most_stable <- stability_analysis %>% head(1)
most_unstable <- stability_analysis %>% tail(1)
print(stability_analysis)
print(paste("The most stable:", most_stable$geo))
print(paste("The most unstable:", most_unstable$geo))
